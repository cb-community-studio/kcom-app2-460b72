...
import borrowhistoryFakerFactory from "./borrowhistoryFakerFactory";
...
const BorrowhistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = borrowhistoryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("borrowhistory").create(elem));
...
export default connect(null, mapDispatch)(BorrowhistoryFakerDialogComponent);
